/* =========================================================
 * patternfly-bootstrap-treeview.js v2.1.5
 * Project URL : https://github.com/patternfly/patternfly-bootstrap-treeview/releases
 * =========================================================
...
/* = Polyfills =============================================
 * Use of Element.closest() method requires a polyfill for IE9, IE10, & IE11 support.
 * See https://developer.mozilla.org/en-US/docs/Web/API/Element/closest
 * We've created ".../js/polyfillClosestMethod_IE9Plus.js"
 * ========================================================= */
...
;(function ($, window, document, undefined) {
	...
	_default.settings = {
		...
		collapseIcon: 'rotate-270',
		loadingIcon: 'glyphicon glyphicon-hourglass',
		emptyIcon: '',
		nodeIcon: '',
		selectedIcon: '',
		checkedIcon: 'ihs-icon-checkbox-checked',
		partiallyCheckedIcon: 'ihs-icon-checkbox-halfchecked',
		uncheckedIcon: 'ihs-icon-checkbox-unchecked',
		tagsClass: 'badge',
		template: {
			tree: $('<ul class="list-group density-tree" role="tree"></ul>'),
			node: $('<li class="list-group-item" role="treeitem"></li>'),
			indent: $('<span class="indent"></span>'),
			icon: {
				node: $('<svg class="icon node-icon"></svg>'),
				expand: $('<svg class="icon expand-icon ihs-icon-arrow"></svg>'),
				check: $('<svg class="icon check-icon" role="checkbox" tabindex="-1"></svg>'),
		...
		styleScopeSelector: '.cpl-scope ', // Set to '' when not needed.
	...
	// CPL default settings overrides
	_default.settings.changedNodeColor = '#39A5DC';
	_default.settings.onhoverColor = '#EAEEF3'; // background-color
	_default.settings.selectedColor = '#FFF';
	_default.settings.selectedBackColor = '#009596';
	_default.settings.searchResultColor = '#D9534F';
	...
	var Tree = function (element, options) {
		...
		this._tabIndexInitialized = false;
	...
	Tree.prototype._init = function (options) {
		...
		// One time need to do a deep extend of options to account for any missing properties the user omits.
		...
		this._load(options)
			...
			.then($.proxy(function () {
				// render to DOM
				this._render(this._options.template);
			}, this));
	};
	...
	*/
	...
	Tree.prototype._setInitialState = function (node, level, done) {
		...
		$.each(node.nodes, $.proxy(function (index, node) {
			...
			if (!node.hasOwnProperty('selectable')) {
				node.selectable = this._options.selectable;
			}

			// if not provided set checkable default value
			if (!node.hasOwnProperty('checkable')) {
				node.checkable = this._options.checkable;
			}
			...
			this._setChecked(node, node.state.checked, this._options);
	...
	Tree.prototype._sortNodes = function () {
		return $.map(Object.keys(this._nodes).sort(function (a, b) {
			...
			this._nodes[value].orderedNodesIndex = index;
	...
	Tree.prototype._keydownHandler = function (event) {
		var target;
		if (event.target.tagName === 'use') {
			target = $(event.target.parentElement);
		} else {
		...
		var $expandEl = target.find('.expand-icon');
		var expandClassList = $expandEl.attr('class') ? $expandEl.attr('class').split(' ') : [];

		var $checkboxEl = target.find('.check-icon');
		var checkboxClassList = $checkboxEl.attr('class') ? $checkboxEl.attr('class').split(' ') : [];

		
		var validKeyFlag = false;
		var validKeyWhitelist = {
			'ArrowDown': true,
			'ArrowLeft': true,
			'ArrowRight': true,
			'ArrowUp': true,
		...
		if (validKeyWhitelist[this._normalizeKeyNames(event.key)]) {
			validKeyFlag = true;
			

			switch (this._normalizeKeyNames(event.key)) {
				case 'ArrowDown':
					this._traverseDownVisible(node);
					break;
				case 'ArrowLeft':
					if ((expandClassList.indexOf('expand-icon') !== -1)) {
						if (node.state.expanded) {
							this._toggleExpanded(node, $.extend({}, _default.options));
						} else if (node.parentId !== undefined) {
							// $(event.currentTarget).find('[data-nodeId="' + node.parentId + '"]').focus();
							this._traverseParent(node);
						}
					} else if (node.parentId !== undefined) {
						// $(event.currentTarget).find('[data-nodeId="' + node.parentId + '"]').focus();
						this._traverseParent(node);
				...
				case 'ArrowRight':
					if ((expandClassList.indexOf('expand-icon') !== -1)) {
						if (!node.state.expanded) {
							...
							this._traverseFirstChild(node);
						}
					}
					break;
				case 'ArrowUp':
					this._traverseUpVisible(node);
					break;
				case 'Enter':
					// Fall through to do the same as Space
				case 'Space':
					// We've made highlightSelected and showCheckbox mutually exclusive
					// When highlightSelected setting === true
					//   Check that showCheckbox !== true
					//   or there is no need to mess with selected state or classes
					//   because we're toggling the checkbox
					// If not toggling selected or toggling checked then fallback to toggling expanded
					if (node.state 
						&& (node.highlightSelected === true || this._options.highlightSelected === true)
						&& !(node.showCheckbox === true || this._options.showCheckbox === true)
						) {
						...
						if(node.selectable) {
							...
							// Fallback of toggling expanded
							this._toggleExpanded(node, $.extend({}, _default.options));
						}

					} else if ((checkboxClassList.indexOf('check-icon') !== -1)) {
						if (node.checkable) {
							// Toggling checkbox
							this._toggleChecked(node, $.extend({}, _default.options));
						} else {
					...
					break;
				case 'Star':
					this._toggleExpandedSelfAndSiblings(node);
					break;
	...
	Tree.prototype.targetNode = function (target) {
		...
		if(typeof(nodeId) === 'undefined' && target[0].tagName === 'path') {
			// Microsoft Edge has issues with click events attached to "use" elements inside "svg" elements so change to the parent "svg" element instead.
			//   The need for this is mostly abated by the "pointer-events: none;" style we applied to all "svg > use" elements.
			target = target[0].parentNode;
			nodeId = target.closest('li.list-group-item').attr('data-nodeId');
		}
	...
	/**
	 * @param {bool} state - Expanded state to set - e.g. make it expanded when true
	 */
	Tree.prototype._setExpanded = function (node, state, options) {
		...
		if (state && node.nodes) { // Expanding
			...
			// Set element
			if (node.$el) {
				node.$el.attr('aria-expanded', 'true')
			...
			if (node.$el) {
				// Can expand?
				if (node.nodes) {
					node.$el.attr('aria-expanded', 'false');
				} else {
					node.$el.removeAttr('aria-expanded');
				}
	...
	Tree.prototype._setSelected = function (node, state, options) {
		...
		// highlightSelected option must === true
		//   If not there is no need to mess with selected state or classes
		//   Because we've made highlightSelected and showCheckbox mutually exclusive the same goes for 
		//   if showCheckbox === true then don't mess with selected state or classes
		if (state 
			&& (node.highlightSelected === true || this._options.highlightSelected === true)
			&& !(node.showCheckbox === true || this._options.showCheckbox === true)
			) {
	...
	Tree.prototype._setChecked = function (node, state, options) {
		...
		// We never pass options when rendering, so the only time
		// we need to validate state is from user interaction
		if (options && state === node.state.checked) {
			return;
		}
	...
	Tree.prototype._render = function (template) {
		if (!this._initialized) {
			// Set template override if provided by options
	...
	Tree.prototype._renderNode = function (node, previousNode) {
		...
		// 1st node gets the only tabindex="0" setting - makes it a grouped control
		if (this._tabIndexInitialized === false) {
			node.$el.attr('tabindex', '0');
			this._tabIndexInitialized = true;
		} else {
		...
		if (node.nodes || node.lazyLoad) {
			// Set ARIA attributes and add expand icon
			node.$el.attr('aria-expanded', 'false')
				.append(this._template.icon.expand.clone());
		} else {
			// Add empty expand/collapse indent/spacer to the text of non-expandable nodes for alignment purposes.
			//   Through template configuration this can be used to represent a terminal leaf node via a document, 
			//   bookmark, or user icon in those types of lists. 
			node.$el.append(this._template.icon.empty.clone());
		}
	...
	Tree.prototype._buildStyle = function () {
		var elementSelector = this._options.styleScopeSelector + '.node-' + this._elementId;
		var style = elementSelector + '{';
		var focusDefaultStyle, hoverFocusStyle;
		...
		if (this._options.onhoverColor) {
			if (this._options.onhoverColor === 'transparent') {
				// Still need focus to work, otherwise keyboard interactions are difficult to understand.
				style += elementSelector + ':hover:not(.node-disabled):not(.node-selected),' + 
					elementSelector + '.hover:not(.node-disabled):not(.node-selected),' + 
					'{' +
					'background-color:' + this._options.onhoverColor + ';' +
				'}';
				style += elementSelector + ':focus:not(.node-disabled):not(.node-selected),' + 
					elementSelector + '.focus:not(.node-disabled):not(.node-selected)' + 
					'{' +
					'background-color:' + _default.settings.onhoverColor + ';' +
				'}';
			} else {
				style += elementSelector + ':hover:not(.node-disabled):not(.node-selected),' + 
					elementSelector + '.hover:not(.node-disabled):not(.node-selected),' + 
		...
		if (this._options.highlightSearchResults && (this._options.searchResultColor || this._options.searchResultBackColor)) {
			...
			style += elementSelector + '.node-result{' + innerStyle + '}';
			style += elementSelector + '.node-result:hover{' + innerStyle + '}';
		}

		// Style selected nodes. highlightSelected and showCheckbox are mutually exclusive.
		if (this._options.highlightSelected && !this._options.showCheckbox && (this._options.selectedColor || this._options.selectedBackColor)) {
			...
			style += elementSelector + '.node-selected{' + innerStyle + '}';
			style += elementSelector + '.node-selected:hover{' + innerStyle + '}';
		}

		// Style changed nodes
		if (this._options.highlightChanges) {
			var innerStyle = 'color: ' + this._options.changedNodeColor + ';';
			style += elementSelector + '.node-check-changed{' + innerStyle + '}';
		}
		...
		$.each(this._orderedNodes, $.proxy(function (index, node) {
			if (node.color || node.backColor) {
				...
				style += elementSelector + '[data-nodeId="' + node.nodeId + '"]{' + innerStyle + '}';
			}

			if (node.iconColor) {
				var innerStyle = 'color:' + node.iconColor + ';';
				style += elementSelector + '[data-nodeId="' + node.nodeId + '"] .node-icon{' + innerStyle + '}';
			}
		}, this));
	...
	Tree.prototype._template = {
		...
		image: $('<span class="image"></span>'),
		badge: $('<span class="badge badge-default"></span>'),
		text: $('<span class="text"></span>')
	};

	Tree.prototype._css = '.treeview .list-group-item{cursor:pointer}.treeview .indent{margin-left:10px;margin-right:10px}.treeview .icon{width:12px;margin-right:5px}.treeview .node-disabled{color:silver;cursor:not-allowed}'
	...
	*/
	Tree.prototype.search = function (pattern, options) {
		...
		// Reveal hidden nodes
		if (results && results.length > 0 && options.revealResults) {
	...
	*/
	Tree.prototype._findNodes = function (pattern, attribute, modifier) {
		...
		modifier = modifier || 'g';
		if(typeof(this._orderedNodes) !== 'undefined') {
	...
	*/
	...
	/**
	 * Normalize the keynames of keydown events to work correctly with dot notation.
	 * @param {String} keyname - Typically the key name given by 'event.key' of a keydown event.
	 * @returns {String} Normalized keyname that can be used in dot notation.
	 */
	Tree.prototype._normalizeKeyNames = function (keyname) {
		var keynameExceptions = {
			'Down': 'ArrowDown', // MS Edge
			'Left': 'ArrowLeft', // MS Edge
			'Multiply': 'Star', // MS Edge
			'Right': 'ArrowRight', // MS Edge
			' ': 'Space',
			'Spacebar': 'Space', // Older browsers like IE (9,10,11), FF 36-
			'*': 'Star',
			'Up': 'ArrowUp' // MS Edge
		};

		if (keynameExceptions[keyname] !== undefined) {
			return keynameExceptions[keyname];
		}
	...
	/**
	 * Upwards traversal. Move from the fromNode to the toNode (or previous node in the _orderedNodes list) that is visible. Roving rules apply to wrap around the list.
	 * @param {Object} fromNode - The current node from which to begin traversal.
	 * @param {optional bool} executeFocusChange - Whether or not to actually change the focus to the toNode that is returned. Default is true. When set to false essentially changes this method into a _getTraverseUpVisible() method.
	 * @returns {Object} toNode that is visible. null if none found.
	 */
	Tree.prototype._traverseUpVisible = function (fromNode, executeFocusChange) {
		// Account for params
		if (!fromNode) return null;
		
		var changingFocus = executeFocusChange;
		if (executeFocusChange !== false && executeFocusChange !== true) {
			changingFocus = true;
		}


		var startingIndex = fromNode.orderedNodesIndex;
		var currentIndex = startingIndex > 0 ? startingIndex - 1: this._orderedNodes.length - 1;
		var toNode = null;

		// From currentIndex through to the 1st item in the list
		for (currentIndex; currentIndex > -1; currentIndex--) {
			if (this._orderedNodes[currentIndex].state.visible) {
				toNode = this._orderedNodes[currentIndex];
				break;
			}
		}

		// Applying roving wraparound rules
		//   From last item in the list through to the currentIndex
		if (toNode === null) {
			currentIndex = this._orderedNodes.length - 1;
			for (currentIndex; currentIndex > startingIndex; currentIndex--) {
				if (this._orderedNodes[currentIndex].state.visible) {
					toNode = this._orderedNodes[currentIndex];
					break;
				}
	...
	/**
	 * Downwards traversal. Move from the fromNode to the toNode (or next node in the _orderedNodes list) that is visible. Roving rules apply to wrap around the list.
	 * @param {Object} fromNode - The current node from which to begin traversal.
	 * @param {optional bool} executeFocusChange - Whether or not to actually change the focus to the toNode that is returned. Default is true. When set to false essentially changes this method into a _getTraverseDownVisible() method.
	 * @returns {Object} toNode that is visible. null if none found.
	 */
	Tree.prototype._traverseDownVisible = function (fromNode, executeFocusChange) {
		// Account for params
		if (!fromNode) return null;
		...
		var startingIndex = fromNode.orderedNodesIndex;
		var currentIndex = startingIndex < this._orderedNodes.length - 1 ? startingIndex + 1: 0;
		var toNode = null;

		// From currentIndex through to the last item in the list
		for (currentIndex; currentIndex < this._orderedNodes.length; currentIndex++) {
			if (this._orderedNodes[currentIndex].state.visible) {
				toNode = this._orderedNodes[currentIndex];
				break;
			}
	...
	/**
	 * Traversal up the heirarchy to the parent node. Move from the fromNode to the toNode (or parent node in the _orderedNodes list).
	 * @param {Object} fromNode - The current node from which to begin traversal.
	 * @param {optional bool} executeFocusChange - Whether or not to actually change the focus to the toNode that is returned. Default is true. When set to false essentially changes this method into a _getTraverseParent() method.
	 * @returns {Object} toNode (parent node). null if none found.
	 */
	Tree.prototype._traverseParent = function (fromNode, executeFocusChange) {
		// Account for params
		if (!fromNode) return null;
		...
		var $toNode = null;
		$toNode = this.$element.find('[data-nodeId="' + fromNode.parentId + '"]');
		// TODO return a real node object instead of the jQuery version.
		//var toNode;
	...
	/**
	 * Traversal down the heirarchy to the 1st child node. Move from the fromNode to the toNode (or 1st child node in the _orderedNodes list).
	 * @param {Object} fromNode - The current node from which to begin traversal.
	 * @param {optional bool} executeFocusChange - Whether or not to actually change the focus to the toNode that is returned. Default is true. When set to false essentially changes this method into a _getTraverseParent() method.
	 * @returns {Object} toNode (parent node). null if none found.
	 */
	Tree.prototype._traverseFirstChild = function (fromNode, executeFocusChange) {
		// Account for params
		if (!fromNode) return null;
	...
	/**
	 * When expanding assure that this node and all of its siblings are expanded.
	 *   If they are already all expanded then we're collapsing so collapse them all.
	 * @param {Object} node - The current node.
	 * @returns {Object} node when successful. null if fail.
	 */
	Tree.prototype._toggleExpandedSelfAndSiblings = function (node) {
		// Account for params
		if (!node) return null;

		var nodeLevel = node.level;
		var foundNodes, alreadyExpandedNodes;
		
		
		if (nodeLevel === 1) {
			// Root nodes - have no parents
			foundNodes = this._findNodes(nodeLevel, 'level');
			// Only the expandable ones
			foundNodes = foundNodes.filter(function (node) {
				return typeof(node.nodes) !== 'undefined' && node.nodes.length > 0;
			});

			alreadyExpandedNodes = foundNodes.filter(function (node) {
				return node.state.expanded === true;
			});

			if (foundNodes.length === alreadyExpandedNodes.length) {
				// Collapsing
				$.each(foundNodes, $.proxy(function (index, node) {
					this._setExpanded(node, false, $.extend({}, _default.options, this._options));
				}, this));
			} else {
				// Expanding
				$.each(foundNodes, $.proxy(function (index, node) {
					this._setExpanded(node, true, $.extend({}, _default.options, this._options));
				}, this));
			}
		} else {
			// Need an exact match on node.parentId
			foundNodes = this._findNodes('^' + node.parentId + '$', 'parentId');
			// Only the expandable ones
			foundNodes = foundNodes.filter(function (node) {
				return typeof(node.nodes) !== 'undefined' && node.nodes.length > 0;
			});
